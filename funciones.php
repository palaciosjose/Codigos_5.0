...
class EmailSearchEngine {
    ...
    public function searchEmails($email, $platform, $user_id = null) {
        ...
        if (!$this->hasUserSubjectAccess($user_id ?? 0, $platform, $subjects)) {
            return $this->createErrorResponse('No tienes permisos para buscar en esta plataforma.');
        }
...
private function checkEmailPermission($email) {
    ...
    private function hasUserSubjectAccess($user_id, $platform, $subject_keywords) {
        $enabled = ($this->settings['USER_SUBJECT_RESTRICTIONS_ENABLED'] ?? '0') === '1';
        if (!$enabled || ($_SESSION['user_role'] ?? '') === 'admin') {
            return true;
        }
        $platform_id = $this->getPlatformId($platform);
        if (!$platform_id) return false;
        $allowed = $this->getUserPlatformSubjects($user_id, $platform_id);
        if (empty($allowed)) return true;
        foreach ($subject_keywords as $subj) {
            if (in_array($subj, $allowed)) return true;
        }
        return false;
    }
